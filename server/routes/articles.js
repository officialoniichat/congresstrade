...
import { authenticateToken, authorizeRole } from '../middleware/auth/index.js';
...
router.post('/', authenticateToken, authorizeRole(['admin', 'manager']), async (req, res) => {
  try {
    ...
    logger.info('Article created successfully', { articleId: result.lastID });
...
router.put('/:id', authenticateToken, authorizeRole(['admin', 'manager']), async (req, res) => {
  try {
    ...
    logger.info('Article updated successfully', { articleId: req.params.id });
...
router.delete('/:id', authenticateToken, authorizeRole(['admin']), async (req, res) => {
  try {
    ...
    logger.info('Article deleted successfully', { articleId: req.params.id });
    res.json({ success: true });
  } catch (error) {
